...
Matrix Matrix::getMat(int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
    ...
    if (i1 < 0 || i2 >= m || j1 < 0 || j2 >= n || i2 < i1 || j2 < j1) {
        cerr << "ERROR: Cannot get submatrix [" << i1 << ".." << i2 << "] x ["
             << j1 << ".." << j2 << "]"
...
Matrix Matrix::diag(const Matrix& M) {
    ...
    cout << "ERROR: Trying to create diagonal matrix from vector of size ("
         << M.m << "x" << M.n << ")" << endl;
    exit(0);
}
...
Matrix Matrix::reshape(const Matrix& M, int32_t m_, int32_t n_) {
    if (M.m * M.n != m_ * n_) {
        cerr << "ERROR: Trying to reshape a matrix of size (" << M.m << "x"
             << M.n << ") to size (" << m_ << "x" << n_ << ")" << endl;
        exit(0);
    }
    Matrix M2(m_, n_);
...
Matrix Matrix::operator-(const Matrix& M) {
    ...
    const Matrix& B = M;
    if (A.m != B.m || A.n != B.n) {
        cerr << "ERROR: Trying to subtract matrices of size (" << A.m << "x"
             << A.n << ") and (" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
    Matrix C(A.m, A.n);
    for (int32_t i = 0; i < m; i++)
...
Matrix Matrix::operator*(const Matrix& M) {
    ...
    const Matrix& B = M;
    if (A.n != B.m) {
        cerr << "ERROR: Trying to multiply matrices of size (" << A.m << "x"
             << A.n << ") and (" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
    Matrix C(A.m, B.n);
...
Matrix Matrix::operator/(const Matrix& M) {
    ...
    if (A.m == B.m && A.n == B.n) {
        ...
        cerr << "ERROR: Trying to divide matrices of size (" << A.m << "x"
             << A.n << ") and (" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
}
...
FLOAT Matrix::det() {
    if (m != n) {
        cerr << "ERROR: Trying to compute determinant of a matrix of size ("
             << m << "x" << n << ")" << endl;
        exit(0);
    }
...
bool Matrix::solve(const Matrix& M, FLOAT eps) {
    ...
    if (A.m != A.n || A.m != B.m || A.m < 1 || B.n < 1) {
        cerr << "ERROR: Trying to eliminate matrices of size (" << A.m << "x"
             << A.n << ") and (" << B.m << "x" << B.n << ")" << endl;
        exit(0);
    }
...
bool Matrix::lu(int32_t* idx, FLOAT& d, FLOAT eps) {
    if (m != n) {
        cerr << "ERROR: Trying to LU decompose a matrix of size (" << m << "x"
             << n << ")" << endl;
        exit(0);
    }
    ...
    for (j = 0; j < n;
         j++) {  // This is the loop over columns of Crout’s method.
        for (i = 0; i < j;
             i++) {  // This is equation (2.3.12) except for i = j.
            sum = val[i][j];
...
void Matrix::svd(Matrix& U2, Matrix& W, Matrix& V) {
    ...
    for (i = n - 1; i >= 0;
         i--) {  // Accumulation of right-hand transformations.
        if (i < n - 1) {
            if (g) {
                for (j = l; j < n;
