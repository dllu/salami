...
vector<errors> calcSequenceErrors(vector<Matrix>& poses_gt,
    ...
    for (size_t first_frame = 0; first_frame < poses_gt.size();
        ...
        for (int32_t i = 0; i < num_lengths; i++) {
            ...
            Matrix pose_delta_result = Matrix::inv(poses_result[first_frame]) *
...
void savePathPlot(vector<Matrix>& poses_gt, vector<Matrix>& poses_result,
    ...
    for (size_t i = 0; i < poses_gt.size(); i += step_size)
        fprintf(fp, "%f %f %f %f\n", poses_gt[i].val[0][3],
                poses_gt[i].val[2][3], poses_result[i].val[0][3],
...
void plotPathPlot(string dir, vector<int32_t>& roi, int32_t idx) {
    ...
    for (int32_t i = 0; i < 2; i++) {
        ...
        fprintf(fp,
                ...
                idx);
        fprintf(fp,
                "\"%02d.txt\" using 3:4 lc rgb \"#0000FF\" title 'Visual "
                "Odometry' w lines,",
                idx);
        fprintf(fp,
                "\"< head -1 %02d.txt\" using 1:2 lc rgb \"#000000\" pt 4 ps 1 "
...
void plotErrorPlots(string dir, char* prefix) {
    ...
    for (int32_t i = 0; i < 4; i++) {
        ...
        system(command);
        sprintf(command, "cd %s; pdfcrop %s_%s_large.pdf %s_%s.pdf",
                dir.c_str(), prefix, suffix, prefix, suffix);
        system(command);
        sprintf(command, "cd %s; rm %s_%s_large.pdf", dir.c_str(), prefix,
                suffix);
        system(command);
    }
}
...
bool eval(const string& gt_dir) {
    // ground truth and result directories
    const string result_dir = "poses/";
    const string error_dir = result_dir + "/errors";
    const string plot_path_dir = result_dir + "/plot_path";
    const string plot_error_dir = result_dir + "/plot_error";
    ...
    for (int32_t i = 0; i <= 10; i++) {
        ...
        // plot status
        printf("Processing: %s, poses: %lu/%lu", file_name, poses_result.size(),
               poses_gt.size());
...
int32_t main(int32_t argc, char* argv[]) {
    ...
    if (argc != 2 && argc != 4) {
        cout << "Usage: ./eval_odometry /path/to/kitti/poses/" << endl;
        return 1;
    }
